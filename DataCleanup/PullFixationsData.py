...
def calculate_fixations_per_minute(file_path):
    """Calculate fixations per minute from a given data file."""
    try:
        # Load available sheet names
        xls = pd.ExcelFile(file_path)
        if 'fixations' not in xls.sheet_names:
            print(f"Warning: 'fixations' sheet not found in {file_path}. Skipping file.")
            return 0
        
        df = pd.read_excel(xls, sheet_name='fixations')
        ...
        for col in possible_columns:
            if col in df.columns:
                df[col] = pd.to_datetime(df[col])
                total_duration_minutes = (df[col].max() - df[col].min()).total_seconds() / 60
                fixations_per_minute = len(df) / total_duration_minutes if total_duration_minutes > 0 else 0
...
def process_all_users(data_directory, output_file):
    """Iterate through user folders, process blinks and fixations data, and save results."""
    results = []
    ...
    for user_folder in os.listdir(data_directory):
        ...
        if os.path.isdir(user_path):
            ...
            if os.path.exists(file_path):
                ...
                fixations_per_min = calculate_fixations_per_minute(file_path)
                results.append({"User": user_folder, "Blinks Per Minute": blinks_per_min, "Fixations Per Minute": fixations_per_min})
...
output_file = "blinks_fixations_summary.csv"
